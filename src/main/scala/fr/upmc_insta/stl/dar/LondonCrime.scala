...
object LondonCrime {
  ...
  if(arg.length <1){
    ...
    val crimeByBorough = londonCrimes
      .groupBy("major_category","borough","year")
      .sum("value")
      .groupBy("major_category","borough")
      .avg("sum(value)")
      .withColumnRenamed("avg(sum(value))","avgCrimesPerYear")
      .withColumn("avgCrimesPerYear", $"avgCrimesPerYear".cast(IntegerType))
      .select("borough", "major_category","avgCrimesPerYear")
    ...
    // ************ TAUX DE CRIMINALITE PAR ARRONDISSEMENT (en pourcentage) ******************* //
    ...
    val criminality = crimeByBorough
      .groupBy("borough")
      .sum("avgCrimesPerYear")
      .withColumnRenamed("sum(avgCrimesPerYear)", "avgCrimesPerYear").as("cb")
